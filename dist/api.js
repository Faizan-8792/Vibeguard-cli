...
export async function generateContextForEditor(task, options) {
    const projectRoot = resolve(options?.cwd || process.cwd());
    ...
    // Load or build graph
    let graphData = await store.read('graph.json');
    if (!graphData) {
    ...
    const tagsData = await store.read('tags.json');
    const tags = tagsData
        ? tagsData.tags
        : await computeTags(projectRoot, graphNodes, config);
    // Load or compute importance
    const importanceData = await store.read('importance.json');
    const importanceScores = importanceData
    ...
    return generateContextPackage(task, selectionResult.selectedFiles, selectionResult.tokenEstimates, totalEstimate.tokens, projectRoot, graphData);
}
export function serializeContextPackageForAgent(pkg) {
