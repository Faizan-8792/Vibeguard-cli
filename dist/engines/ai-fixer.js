 ...
 */
export async function generateFixes(projectRoot, credentials, attackFindings, securityIssues, opts = {}) {
...
function groupIssuesByFile(attackFindings, securityIssues) {
    const issuesByFile = new Map();
    const add = (file, description) => {
        const existing = issuesByFile.get(file);
        if (existing)
            existing.push(description);
        else
            issuesByFile.set(file, [description]);
    };
 ...
 */
async function planFileFix(client, projectRoot, file, issues, maxTokensPerFile) {
    let originalContent;
    try {
 ...
 */
export async function applyFixes(projectRoot, plans) {
