 ...
 */
async function buildCompactContext(projectRoot, attackFindings, securityIssues, maxExcerptLines) {
    const parts = [];
    ...
    const severeFiles = new Map();
    for (const f of [...attackFindings].sort(severityRank).slice(0, 10)) {
        if (!severeFiles.has(f.file))
            severeFiles.set(f.file, new Set());
...
export async function runAIAdvisor(projectRoot, credentials, attackFindings, securityIssues, opts = {}) {
...
//# sourceMappingURL=ai-security-advisor.js.map