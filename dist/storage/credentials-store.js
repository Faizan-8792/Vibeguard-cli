 ...
 */
export function inferProvider(apiKey) {
    ...
    if (apiKey.startsWith('sk-'))
        return 'openai'; // generic OpenAI-style key (also DeepSeek/Moonshot use sk-)
    return 'openrouter';
}
...
function resolveCredentialsFromEnv() {
    ...
    if (!apiKey)
        return null;
    const provider = process.env['VIBEGUARD_PROVIDER'] ?? inferProvider(apiKey);
