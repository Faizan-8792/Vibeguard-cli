...
export async function runTrash(ctx, opts) {
    ...
    switch (opts.action) {
        ...
        case 'restore': {
            if (!opts.target) {
                throw new VibeguardError(ErrorCodes.CONFIG_INVALID, 'Restore requires an ID or path. Usage: vibeguard trash restore <id|path>');
            }
            try {
                ...
                throw new VibeguardError(ErrorCodes.RESTORE_CONFLICT, err instanceof Error ? err.message : 'Restore failed');
            }
            break;
        }
        case 'purge': {
            if (!opts.yes) {
                throw new VibeguardError(ErrorCodes.LIMIT_EXCEEDED, 'Purge requires --yes flag to confirm deletion of all trash entries.');
            }
        ...
        default:
            throw new VibeguardError(ErrorCodes.UNKNOWN_COMMAND, `Unknown trash action: "${opts.action}". Valid actions: list, restore, purge`);
    }
}
