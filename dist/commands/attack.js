...
export async function runAttack(ctx, opts) {
...
async function emitAttackJson(ctx, opts, attackResult, securityResult) {
...
async function renderAttackTerminal(ctx, opts, attackResult, securityResult) {
...
async function applyTerminalFixes(ctx, opts, credentials, attackResult, securityResult) {
    const { logger, projectRoot } = ctx;
    ...
    try {
        ...
        if (changed.length === 0) {
            ...
            const out = ['', `  ${brand.primary.bold('Proposed Fixes (dry-run)')}`, ''];
...
// ─── Shared AI helpers ───────────────────────────────────────────────────────
async function resolveCredentials(projectRoot) {
    const credentials = await new CredentialsStore(projectRoot).resolve();
    if (!credentials) {
        throw new VibeguardError(ErrorCodes.CONFIG_NOT_FOUND, 'No LLM API key configured. Run `vibeguard config set-key <key>` first, or set VIBEGUARD_API_KEY.');
    }
    return credentials;
}
async function runAdvisor(projectRoot, credentials, attackResult, securityResult, budget) {
...
async function generateChangedFixPlans(projectRoot, credentials, attackResult, securityResult) {
...
async function applyFixPlans(projectRoot, plans) {
...
// ─── Terminal rendering ──────────────────────────────────────────────────────
function printLocalFindings(attackResult, opts) {
    const output = [];
    ...
    if (attackResult.findings.length === 0) {
        ...
        for (const f of attackResult.findings) {
            if (!byType.has(f.attackType))
                byType.set(f.attackType, []);
...
function printAISection(aiResult) {
    const output = [];
    output.push('');
    output.push(divider());
    output.push('');
