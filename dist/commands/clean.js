...
export async function runClean(ctx, opts) {
    ...
    if (opts.apply) {
        // Load existing plan
        const fileStore = new FileStoreImpl(projectRoot);
        const plan = await fileStore.read('cleanup-plan.json');
        if (!plan) {
            throw new VibeguardError(ErrorCodes.CONFIG_NOT_FOUND, 'No cleanup plan found. Run `vibeguard clean --plan` first.');
        }
        const fileCandidates = plan.candidates.filter((c) => c.kind === 'file');
        // Enforce limits
        if (fileCandidates.length > config.clean.maxChangesPerRun && !opts.force) {
            throw new VibeguardError(ErrorCodes.LIMIT_EXCEEDED, `Cleanup plan has ${fileCandidates.length} file candidates, exceeding limit of ${config.clean.maxChangesPerRun}. Use --force to override.`, { count: fileCandidates.length, limit: config.clean.maxChangesPerRun });
        }
        ...
        if (opts.dryRun) {
            ...
            if (options.json) {
                emitJson({
                    dryRun: true,
                    plannedChanges: fileCandidates.map((c) => ({
                        type: 'move',
                        path: c.path,
