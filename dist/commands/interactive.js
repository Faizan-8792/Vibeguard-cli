...
async function runSecurityInteractive(ctx) {
...
async function runHealthInteractive(ctx) {
...
async function runMapInteractive(ctx) {
...
async function runDeadInteractive(ctx) {
...
async function runPackInteractive(ctx) {
...
async function runTrashInteractive(ctx) {
...
async function runInitInteractive(ctx) {
...
async function copyFixInstructionsToClipboard(issues) {
    const clipText = [];
...
async function runReportInteractive(ctx) {
    ...
    if (devDependencies.includes('jest'))
        stack.push('Jest');
    if (dependencies.includes('prisma') || dependencies.includes('@prisma/client'))
        stack.push('Prisma');
...
async function runAttackInteractive(ctx) {
    ...
    if (action === 'ai') {
        ...
        try {
            ...
            const fixAction = await selectPrompt({
...
async function runLLMConfigInteractive(ctx) {
...
async function runAIFixFlow(ctx, credentials, attackFindings, securityIssues, previewOnly) {
