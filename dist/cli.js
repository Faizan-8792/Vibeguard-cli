...
async function createContext(opts, commandName) {
    ...
    const config = await loadConfig(projectRoot, opts.config, opts.include || [], opts.exclude || []);
    return { options: opts, config, logger, projectRoot };
}
function resolveShorthand(opts) {
    if (opts.run) {
        return {
            command: 'interactive',
            async run(ctx) {
...
function setupProgram() {
    ...
    program
        ...
        if (action === 'install') {
            ...
            throw new VibeguardError(ErrorCodes.UNKNOWN_COMMAND, `Unknown action: "${action}". Use: vibeguard kiro install | uninstall`);
        }
    });
    // cursor command
    program
        ...
        const { runInstall, runUninstall } = await import('./commands/install.js');
        if (action === 'install')
            await runInstall(ctx, { platform: 'cursor' });
        else if (action === 'uninstall')
            await runUninstall(ctx, { platform: 'cursor' });
        else
            throw new VibeguardError(ErrorCodes.UNKNOWN_COMMAND, `Unknown action. Use: vibeguard cursor install | uninstall`);
    });
    // claude command
    program
        .command('claude')
        ...
        const { runInstall, runUninstall } = await import('./commands/install.js');
        if (action === 'install')
            await runInstall(ctx, { platform: 'claude' });
        else if (action === 'uninstall')
            await runUninstall(ctx, { platform: 'claude' });
        else
            throw new VibeguardError(ErrorCodes.UNKNOWN_COMMAND, `Unknown action. Use: vibeguard claude install | uninstall`);
    });
    // install command
    program
        .command('install')
...
async function main() {
    ...
    try {
        ...
        if (err instanceof VibeguardError) {
            ...
            const internalError = new VibeguardError(ErrorCodes.INTERNAL_ERROR, err instanceof Error ? err.message : 'Unknown error');
            if (isJson) {
