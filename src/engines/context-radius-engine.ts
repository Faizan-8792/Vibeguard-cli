...
export async function selectContext(
  ...
  opts: { radius?: number; budget?: number; mode?: PackMode }
  ...
  // Expand by radius
  const expanded = new Map<string, { score: number; hopDistance: number }>();
  ...
  for (let hop = 1; hop < radius; hop++) {
    ...
    for (const [filePath] of currentFiles) {
      ...
      for (const neighbor of neighbors) {
        if (!graphNodes.has(neighbor)) continue;
...
function stemWord(word: string): string {
  ...
  else if (result.endsWith('able')) result = result.slice(0, -4);
  else if (result.endsWith('ible')) result = result.slice(0, -4);
  else if (result.endsWith('ful')) result = result.slice(0, -3);
  else if (result.endsWith('less')) result = result.slice(0, -4);
  else if (result.endsWith('ly')) result = result.slice(0, -2);
  else if (result.endsWith('ed')) result = result.slice(0, -2);
  else if (result.endsWith('er')) result = result.slice(0, -2);
  else if (result.endsWith('es')) result = result.slice(0, -2);
  else if (result.endsWith('s') && !result.endsWith('ss')) result = result.slice(0, -1);

  return result.length > 1 ? result : word;
}

function applyModeMultipliers(
  ...
  for (const [filePath, score] of scores) {
    ...
    switch (mode) {
      case 'feature':
        if (entry?.routeUsage) multiplier = 1.5;
        break;
      case 'bugfix':
        if (entry && entry.gitCommits > 5) multiplier = 1.5;
        break;
      case 'refactor': {
        const node = graphNodes.get(filePath);
        if (node && node.dependents.length > 10) multiplier = 1.5;
        break;
      }
    }
