...
- **VibeGuard_CLI**: The top-level Node.js binary `vibeguard` exposed via `commander`, which routes invocations to subcommand handlers.
- **Doctor_Command**: The `vibeguard doctor` subcommand that aggregates findings into a Project Health Score.
- **Security_Command**: The `vibeguard security` subcommand that runs the Security_Scanner and applies opt-in fixes.
- **Clean_Command**: The `vibeguard clean` subcommand that detects dead code and stages cleanup actions.
- **Map_Command**: The `vibeguard map` subcommand that builds and persists the project dependency graph.
- **Pack_Command**: The `vibeguard pack` subcommand that produces a focused context package for a given task.
- **Trash_Command**: The `vibeguard trash` subcommand group (`list`, `restore`, `purge`) for managing soft-deleted artifacts.
- **Init_Command**: The `vibeguard init` subcommand that scaffolds the local configuration file.
...
- **Tagging_Engine**: The module that derives kebab-case tags for source files from identifiers, paths, framework patterns, and explicit `@vibeguard:` comments.
...
- **File_Store**: The persistence layer responsible for reading and writing artifacts under `.vibeguard/`.
- **Trash_Store**: The persistence layer responsible for soft-deleted artifacts under `.vibeguard-trash/`.
- **Config_Store**: The persistence layer responsible for `.vibeguard/config.json` defaults, overrides, and validation.
...
- **Context_Package**: The pair of artifacts `.vibeguard/context-package.md` and `.vibeguard/context-package.json` produced by Pack_Command.
...
**User Story:** As a developer, I want a single `vibeguard` binary that exposes consistent global options across all subcommands, so that I can use the tool the same way in every project and from every AI agent.

#### Acceptance Criteria

1. THE VibeGuard_CLI SHALL register the subcommands `doctor`, `security`, `clean`, `map`, `pack`, `trash`, and `init` using `commander`.
2. THE VibeGuard_CLI SHALL accept the global options `--json`, `--cwd <path>`, `--include <glob...>`, `--exclude <glob...>`, `--config <path>`, `--verbose`, and `--quiet` for every subcommand.
3. WHEN the user invokes `vibeguard --help`, THE VibeGuard_CLI SHALL print a usage summary listing every subcommand and every global option.
4. WHEN the user invokes `vibeguard --version`, THE VibeGuard_CLI SHALL print the version declared in `package.json` and exit with status code 0.
5. WHEN no subcommand is provided, THE VibeGuard_CLI SHALL print the usage summary and exit with status code 0.
6. IF an unknown subcommand or option is provided, THEN THE VibeGuard_CLI SHALL print an error message naming the offending token and exit with status code 2.
7. WHILE JSON_Mode is active, THE Logger SHALL write nothing to stdout, and THE VibeGuard_CLI SHALL emit exactly one JSON document to stdout terminated by a newline.
8. WHEN any subcommand completes successfully, THE VibeGuard_CLI SHALL exit with status code 0.
9. IF any subcommand encounters a fatal error, THEN THE VibeGuard_CLI SHALL emit a structured error object `{ "error": { "code": string, "message": string, "details": object? } }` on stdout in JSON_Mode or a single-line message on stderr otherwise, and exit with a non-zero status code.
10. THE VibeGuard_CLI SHALL run on Node.js version 18 or higher on Windows, macOS, and Linux without requiring native compilation.

### Requirement 2: Project Initialization

**User Story:** As a developer onboarding VibeGuard to a project, I want `vibeguard init` to scaffold a documented configuration file, so that I can configure ignores, tag rules, importance weights, security patterns, and model pricing without reading source code.

#### Acceptance Criteria

1. WHEN the user invokes `vibeguard init`, THE Init_Command SHALL create `.vibeguard/config.json` at Project_Root populated with documented default values.
2. THE default `.vibeguard/config.json` SHALL include the keys `ignore`, `tags.customRules`, `importance.weights`, `security.customSecretPatterns`, `context.defaultRadius`, `context.defaultTokenBudget`, and `context.models`.
3. THE default `ignore` array SHALL include the globs `node_modules/**`, `dist/**`, `build/**`, `coverage/**`, `**/*.test.ts`, `**/*.test.tsx`, `**/*.test.js`, `**/*.spec.ts`, `**/*.spec.tsx`, `**/*.spec.js`, `.vibeguard/**`, and `.vibeguard-trash/**`.
...
7. IF `.vibeguard/config.json` already exists when `vibeguard init` is invoked without `--force`, THEN THE Init_Command SHALL leave the existing file unchanged and exit with status code 1 and an explanatory message.
8. WHEN `vibeguard init --force` is invoked, THE Init_Command SHALL overwrite the existing `.vibeguard/config.json` with the documented defaults.
9. WHEN the user invokes `vibeguard init`, THE Init_Command SHALL ensure that the directory `.vibeguard/` exists at Project_Root.
...
1. WHEN any subcommand other than `init` runs, THE Config_Store SHALL load `.vibeguard/config.json` from Project_Root if it exists, or fall back to the documented defaults if it does not.
2. WHEN `--config <path>` is supplied, THE Config_Store SHALL load configuration from the supplied path instead of `.vibeguard/config.json`.
...
**User Story:** As a developer, I want `vibeguard map` to build a stored dependency graph of my project, so that downstream commands can reason about reachability, importance, and context expansion without re-parsing the codebase each time.

#### Acceptance Criteria

1. WHEN the user invokes `vibeguard map`, THE Graph_Builder SHALL produce a graph in which each node has the shape `{ "file": string, "imports": string[], "exports": string[], "dependents": string[] }`.
2. THE Graph_Builder SHALL persist the graph to `.vibeguard/graph.json` together with a sibling `.vibeguard/analysis-meta.json` recording the schema version, build timestamp, and the SHA-256 hash of every file included in the graph.
3. WHEN `.vibeguard/graph.json` does not exist or the recorded schema version does not match the current schema version, THE Graph_Builder SHALL perform a full rebuild.
4. WHEN `.vibeguard/graph.json` exists and the schema version matches, THE Graph_Builder SHALL rebuild only nodes whose source file content hash or modification time has changed since the last run, and SHALL update `dependents` for any node whose imports changed.
...
**User Story:** As a developer, I want `vibeguard security` to detect exposed secrets, missing `.gitignore` entries, and risky framework usage, so that I can find and remediate security issues before they are committed or deployed.

#### Acceptance Criteria

1. WHEN the user invokes `vibeguard security`, THE Security_Scanner SHALL detect that a `.env` file exists at Project_Root and is not covered by `.gitignore`, and SHALL emit an issue with category `secrets-gitignore`.
...
2. WHEN `vibeguard security --fix=gitignore` is invoked, THE Security_Command SHALL ensure that `.gitignore` at Project_Root contains entries for `.env`, `.env.local`, `.vibeguard/`, and `.vibeguard-trash/`, appending only entries that are missing.
3. WHEN `vibeguard security --fix=env` is invoked, THE Security_Command SHALL move every detected hard-coded secret literal whose category is `hard-coded-secret` into a `.env` file at Project_Root, generate or update `.env.example` with placeholder values, and replace the literal in source with a `process.env.<NAME>` reference using a `ts-morph` AST transform.
...
6. WHEN `--git-safe` is supplied together with any `--fix` mode, THE Security_Command SHALL require that `git status --porcelain` is empty, create a new branch named `vibeguard/security-fix-<timestamp>`, apply the fix on that branch, and commit the result with a descriptive message.
7. THE Security_Command SHALL never run a `--fix` operation on files outside Project_Root.

### Requirement 7: Dead Code Detection

**User Story:** As a developer cleaning a codebase, I want `vibeguard clean` to identify unused files, exports, functions, imports, and duplicate React components, so that I can remove dead weight without manually grepping the project.
...
**User Story:** As a developer, I want `vibeguard clean` to produce a reviewable plan and to soft-delete files into a recoverable trash, so that I can clean my codebase without risking data loss.

#### Acceptance Criteria

1. WHEN `vibeguard clean --plan` is invoked, THE Clean_Command SHALL write `cleanup-plan.json` to `.vibeguard/` containing each candidate sorted by ascending Importance_Score, including `path`, `kind` (`file` | `export` | `import` | `duplicate-component`), `importance`, `lastCommitDate`, `testOnlyReferences`, and a stable `id`.
2. WHEN `vibeguard clean --apply` is invoked without `--plan`, THE Clean_Command SHALL load the most recent `.vibeguard/cleanup-plan.json` and apply only the items it contains.
3. WHEN applying a `file` candidate, THE Clean_Command SHALL move the file into `.vibeguard-trash/<uuid>/` mirroring its original path under that directory, and SHALL never invoke a hard-delete syscall.
4. THE Clean_Command SHALL write `meta.json` inside each `.vibeguard-trash/<uuid>/` entry with the keys `id`, `originalPath`, `movedAt`, `importance`, `lastCommitDate`, and `kind`.
5. WHEN `vibeguard clean --interactive` is invoked, THE Clean_Command SHALL group candidates into batches of at most 10 and prompt for confirmation per batch before applying.
...
8. WHEN `--git-safe` is supplied, THE Clean_Command SHALL require a clean working tree, create a branch named `vibeguard/clean-<timestamp>`, perform moves and edits there, and commit the result.

### Requirement 9: Trash Management

**User Story:** As a developer, I want to inspect, restore, and purge soft-deleted artifacts, so that mistakes from `vibeguard clean` are reversible.

#### Acceptance Criteria

1. WHEN the user invokes `vibeguard trash list`, THE Trash_Command SHALL emit one row per `.vibeguard-trash/<uuid>/meta.json` containing `id`, `originalPath`, `movedAt`, `kind`, and `importance`.
2. WHEN the user invokes `vibeguard trash restore <id|path>`, THE Trash_Command SHALL move the corresponding entry back to its `originalPath`, recreating any missing parent directories, and SHALL remove the now-empty `.vibeguard-trash/<uuid>/` directory.
...
4. WHEN the user invokes `vibeguard trash purge`, THE Trash_Command SHALL require either an interactive confirmation prompt of the literal phrase `purge` or the flag `--yes`, and SHALL remove every entry under `.vibeguard-trash/` only after that confirmation.
5. IF `.vibeguard-trash/` does not exist, THEN `vibeguard trash list` SHALL emit an empty list and exit with status code 0.

### Requirement 10: Tagging Engine

**User Story:** As a developer, I want VibeGuard to derive meaningful tags for every source file, so that `pack` can find files relevant to a task without me manually annotating them.
...
4. THE Tagging_Engine SHALL parse `// @vibeguard: tag1, tag2` comments anywhere in a file and add the listed tags verbatim after kebab-case normalization.
...
6. THE Tagging_Engine SHALL persist its output to `.vibeguard/tags.json` as a map `{ [filePath: string]: string[] }` with tags sorted alphabetically and deduplicated.
...
**User Story:** As a developer, I want VibeGuard to score how important each file is to my project, so that cleanup and context-packing prioritize files I actually rely on.
...
6. THE Importance_Analyzer SHALL persist its output to `.vibeguard/importance.json` as a map `{ [filePath: string]: { score: number, dependents: number, imports: number, gitCommits: number, routeUsage: number } }`.
...
1. WHEN the user invokes `vibeguard doctor`, THE Health_Analyzer SHALL run or read cached outputs from Security_Scanner, Dead_Code_Scanner, Graph_Builder, and Context_Radius_Engine before computing scores.
...
**User Story:** As a developer, I want VibeGuard to estimate the token count and per-model cost of any selected file set, so that I can predict the price of an AI request before sending it.
...
**User Story:** As a developer using an AI coding agent, I want `vibeguard pack <task>` to assemble a focused, importance-aware context package within a token budget, so that my agent receives only the files that matter for the task.

#### Acceptance Criteria

1. WHEN the user invokes `vibeguard pack <task>` or `vibeguard pack --task-file <path>`, THE Context_Radius_Engine SHALL normalize the task text by lowercasing, removing the documented English stopword list, and applying the documented Porter-like stemming rules.
2. THE Context_Radius_Engine SHALL match normalized task tokens against tags in `.vibeguard/tags.json` and SHALL produce a seed set of files ranked by `match_score = sum(token_match_weight) × Importance_Score`.
...
7. THE Context_Package_Generator SHALL write `.vibeguard/context-package.md` containing a `Task` section, a `Detected Stack` section inferred from `package.json` dependencies and config files, a `Relevant Files` section with one bullet per selected file (path, tags, importance, role, hop distance), a `Warnings` section listing any selected file with fan-in > 10 or marked critical by Security_Scanner, and a `Token Budget` section with point estimate, range, and reduction percentage versus including all candidate files.
8. THE Context_Package_Generator SHALL write `.vibeguard/context-package.json` containing the same data as the markdown package in machine-readable form.
...
**User Story:** As an AI agent or IDE extension author, I want every VibeGuard JSON contract to be stable and self-describing, so that I can shell out to `vibeguard ... --json` and parse the result without screen-scraping.
...
2. THE VibeGuard_CLI SHALL increment the major component of `schemaVersion` whenever a JSON field is renamed, removed, or has its type changed.
3. THE VibeGuard_CLI SHALL expose a thin, isolated programmatic API including at minimum `runCommand(name, args, options): Promise<Result>`, `generateContextForEditor(task, options): Promise<ContextPackage>`, and `serializeContextPackageForAgent(pkg): string`.
4. THE programmatic API SHALL not call any remote AI API and SHALL not require credentials of any AI provider.
5. THE VibeGuard_CLI SHALL document every JSON contract in `README.md` with at least one example payload per subcommand.
...
**User Story:** As a developer running VibeGuard in CI or against a large repository, I want strong safety guarantees by default, so that the tool can never silently damage my project.

#### Acceptance Criteria

1. THE VibeGuard_CLI SHALL be read-only by default, and no subcommand SHALL modify any file outside `.vibeguard/` and `.vibeguard-trash/` unless an explicit mutation flag (`--fix`, `--apply`, `--restore`, `--purge`, `--force`, or `--init --force`) is supplied.
2. THE VibeGuard_CLI SHALL support `--dry-run` for every mutating subcommand and SHALL emit unified-diff output (or, in JSON_Mode, a structured `plannedChanges[]` array) describing every action that would be taken.
3. THE VibeGuard_CLI SHALL never delete any source file with a hard-delete syscall; deletions SHALL always route through Trash_Store.
4. WHEN `--git-safe` is supplied to a mutating subcommand, THE VibeGuard_CLI SHALL refuse to proceed unless `git status --porcelain` is empty, and SHALL perform mutations on a new branch `vibeguard/<command>-<timestamp>` committed by Git_Utils with a descriptive message.
...
**User Story:** As a developer, I want clear, non-noisy output in the terminal and silent output in JSON mode, so that VibeGuard fits into both interactive and machine workflows.
...
**User Story:** As a developer working in a large repository, I want VibeGuard to finish quickly on subsequent runs, so that I can use it inside an interactive workflow.

#### Acceptance Criteria

1. THE Graph_Builder SHALL store per-file SHA-256 content hashes in `.vibeguard/analysis-meta.json` and SHALL skip re-parsing any file whose hash matches the stored hash.
2. THE Tagging_Engine and Importance_Analyzer SHALL skip recomputation for any file whose graph node was unchanged in the most recent build, reusing the persisted entry from `.vibeguard/tags.json` and `.vibeguard/importance.json`.
...
**User Story:** As a maintainer publishing VibeGuard, I want a clean Node.js package layout with the documented commands, so that users can install it globally and AI agents can locate the binary deterministically.

#### Acceptance Criteria

1. THE `package.json` SHALL declare `"engines": { "node": ">=18" }`, a `bin` entry mapping `vibeguard` to the compiled CLI entrypoint, and a `files` array that excludes test sources and `.vibeguard*` runtime data.
