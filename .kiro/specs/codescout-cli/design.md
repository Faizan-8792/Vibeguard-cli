...
4. Storage layer is the only module that touches the filesystem for `.codescout/` artifacts
...
interface ResolvedConfig extends CodeScoutConfig {
...
All persisted artifacts live under `.codescout/` (analysis data) or `.codescout-trash/` (soft-deleted files).

### `.codescout/config.json`

```json
{
  "ignore": ["node_modules/**", "dist/**", "build/**", "coverage/**", "**/*.test.ts", "**/*.test.tsx", "**/*.test.js", "**/*.spec.ts", "**/*.spec.tsx", "**/*.spec.js", ".codescout/**", ".codescout-trash/**"],
  "tags": {
    "customRules": [
...
### `.codescout/context-package.json`
...
### `.codescout-trash/<uuid>/meta.json`
...
### `codescout trash list|restore|purge`

```mermaid
flowchart TD
    A[Parse subcommand] --> B{list?}
    B -->|Yes| C[Read all meta.json under .codescout-trash/]
    C --> D[Emit entries]
    A --> E{restore?}
    ...
    K -->|Yes| L[Remove all .codescout-trash/ contents]
    K -->|No| M[Abort]
```
...
| `UNKNOWN_COMMAND` | CLI usage error | `codescout foo` |
| `UNKNOWN_OPTION` | CLI usage error | `codescout --baz` |

### Error Propagation

- Domain engines throw typed `CodeScoutError` instances with `code`, `message`, and optional `details`.
...
- **Tagging Engine**: Custom rules and `@codescout:` comments allow LLM agents to annotate files programmatically.
...
*For any* string that is not a registered subcommand or valid option, invoking `codescout <string>` SHALL exit with code 2 and the error message SHALL contain the offending token verbatim.
...
*For any* set of source files where a subset has changed content (different SHA-256 hash), re-running `codescout map` SHALL re-parse only the changed files, reuse cached nodes for unchanged files, and update `dependents` arrays for any node whose `imports` changed. The final graph SHALL be identical to a full rebuild.
...
*For any* command invocation without an explicit mutation flag (`--fix`, `--apply`, `--restore`, `--purge`, `--force`, `--init --force`), no file outside `.codescout/` and `.codescout-trash/` SHALL be created, modified, or deleted.
...
*For any* `.gitignore` file content, applying `--fix=gitignore` twice SHALL produce the same result as applying it once. After application, `.gitignore` SHALL contain entries for `.env`, `.env.local`, `.codescout/`, and `.codescout-trash/`.
...
*For any* file moved to trash via `codescout clean --apply`, invoking `codescout trash restore <id>` SHALL restore the file to its exact original path with identical content, and the trash entry SHALL be removed.
...
*For any* operation performed by CodeScout, no file SHALL be removed via `unlink`, `rmdir`, or equivalent hard-delete syscall on user source files. All removals SHALL route through Trash_Store.
...
```typescript
// Feature: codescout-cli, Property N: <property title>
```
